...
def arccos(x):
    """
    Sometimes rounding errors cause the dot product to be just above or below 
        plus or minus one respectively. This function makes the program ignore
...
def get_rotator(vector, target):
    ...
    axis = axis.normalize()
    angle = arccos(vector.normalize().dot(target.normalize()))
...
def get_second_rotator(origional_target, vector, target):
    ...
    angle = arccos(v1.normalize().dot(v2.normalize()))
    angle = float(angle)
...
def finding_the_targets(found_vectors, theoretical_vectors, crystal, angle_accuracy):
    ...
    if len(found_vectors) == 2:
        ...
        found_angle = float(dnp.rad2deg(arccos(found_dot)))
        for r in range(9, 1, -1):
            ...
            for combination in combinations(correct_length, 2):
                ...
                target_angle = float(dnp.rad2deg(arccos(target_dot)))
                if round(combination[0].length(), r) == round(
                    ...
                    target_dot = combination[0].normalize().dot(
                        combination[1].normalize())
                    target_angle = float(dnp.rad2deg(arccos(target_dot)))
                    if found_angle - angle_accuracy < target_angle < found_angle + angle_accuracy:
        ...
        for r in range(9, 1, -1):
            ...
            for combination in combinations(correct_length, 2):
                if round(combination[0].length(), r) == round(
                    ...
                    target_dot = (combination[0].normalize()).dot(
                        combination[1].normalize())
                    target_angle = float(dnp.rad2deg(arccos(
                        target_dot)))
                    if found_angle-angle_accuracy < target_angle < found_angle + angle_accuracy:
        ...
        log.write("Couldn't find targets.")
        log.write('\n\n\n')
        log.close()
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        f.plot_vectors_as_arrows([found_vectors[0]], fig, ax, color='red')
        f.plot_vectors_as_arrows([found_vectors[1]], fig, ax, color='green')
        f.plot_vectors_as_arrows([found_vectors[2]], fig, ax, color='blue')
        f.plot_sphere(found_vectors[0].length(), fig, ax)
        plt.show()
        raise Exception("""Could not find two vectors with moduli that match
            the theoretical vectors to an accuracy of 2 decimal places and an
            angle between them which matches to the nearest degree.""")
