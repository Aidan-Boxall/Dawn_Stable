...
def test_by_hkl(cif, hkl_list, add_error=True, create_plots=True,  old_filter=False, energy=8):
    mycrys = c.Crystal()
    mycrys.load_cif(cif)
    if old_filter:
        refl = 'allowed'
    ...
    if test_data(mock_data, all_vectors_rot, all_vectors, mycrys, old_filter, vectors, create_plots):
        return True
    else:
        return False
