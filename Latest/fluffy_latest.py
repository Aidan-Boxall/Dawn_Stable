...
class DetectorFactory(object):
  ...
  def __init__(self, obj, beam):
    ...
    self.model = Detector()
    print 'Types:'
    print type(detector_type)
    print type(detector_name)
    print type(fast_axis)
    print type(slow_axis)
    print type(origin)
    print type(pixel_size)
    print type(image_size)
    print type(trusted_range)
    print type(thickness_value)
    print type(material)
    print type(mu)
...
def get_q(filename, pix_coords, image):
  '''
...
#   from dxtbx.model.experiment.experiment_list import Experiment
  ...
  from scitbx import matrix
  detector = experiments['detector']
  beam =experiments['beam']
  goniometer = experiments['goniometer']
  scan = experiments['scan']
  angle = scan.get_angle_from_image_index(image, True)
  s0 = matrix.col(beam.get_s0())
  panel = detector[0]
  
  s1 = matrix.col(panel.get_pixel_lab_coord((pix_coords[0],pix_coords[1]))).normalize() * s0.length()
  q = s1 - s0
  axis = goniometer.get_rotation_axis()
  return {'q':q, 'axis':axis, 'angle':angle}
