...
class Structure:
    ...
    def holo_group(self):
        # !!! Does not give holohedral group !!!
        cc = self.get_cctbx_structure()
        uc = cc.primitive_setting().unit_cell()
        hg = cctbx.sgtbx.lattice_symmetry.group(uc)
        return hg.point_group_type()
...
class Crystal:
    ...
    def load_cif(self,filename,data_code=None):
        ...
        except (CifBuilderError, AttributeError):
            if data_code is not None:
    ...
    def reflection_list(self,energy,refl='sym',anomalous_flag=False,sort='two_theta',print_list=True):
        ...
        if refl=='all':
            ...
            sg = cctbx.sgtbx.space_group_info(self.structure.space_group()).group()
            d_spacing_rounded = [ round(dsp, 4) for dsp in list(d_spacing) ]
            for jj in range(0,len(ind)):
                if sg.crystal_system() != 'Trigonal' or (sg.crystal_system() == 'Trigonal' and abs(sum(self.lattice.parameters()[3:])/3-self.lattice.parameters()[3])>0.001 and ind[jj][2]%3!=0) or (sg.crystal_system() == 'Trigonal' and abs(sum(self.lattice.parameters()[3:])/3-self.lattice.parameters()[3])<0.001 and (2*ind[jj][0]+2*ind[jj][1]+2*ind[jj][2])%3!=0):
                    mult = list(d_spacing_rounded).count(d_spacing_rounded[jj])
                    if mult == 1:
                        equival = cctbx.miller.sym_equiv_indices(sg, ind[jj]).indices()
                        for er in equival:
                            reflist.append([er.h(), inten[jj], 'not calc.', two_theta[jj], d_spacing[jj]])
        # EDIT START
        if reflist == []:
            return reflist
        else:
