...
target_data = [vectors[1], non_equiv[3]]
...
f.plot_vectors(vectors, fig2, ax2)
f.plot_sphere(mock_data[0].modulus(), fig2, ax2)
...
def diff(array, data, vectors):
    ...
    total = 0
    for i, dat in enumerate(data):
        total += (dat.unit() - targets[i].unit()).modulus()
    return total
...
all_vectors_copy = rm.rotate_list(new_U, all_vectors_copy)
