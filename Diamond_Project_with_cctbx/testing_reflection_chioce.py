...
def test_data(mock_data, all_vectors_rot, all_vectors, mycrys,i,j,k, old_filter, vectors=False, plot=False):
    if not old_filter:
...
#     f.plot_vectors(final_vectors, fig, ax)
    ...
    log = open('test_log_ignore.txt', 'a')
...
#     f.plot_vectors(all_vectors_rot, fig, ax)
    ...
    if test_data(mock_data, all_vectors_rot, all_vectors, mycrys,i, j, k, old_filter, vectors, True):
        return True
    else:
        return False
