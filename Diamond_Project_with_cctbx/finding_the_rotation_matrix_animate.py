...
def get_rotator(vector, target):
    ...
    return Rotator(scm.r3_rotation_axis_and_angle_as_matrix(axis, angle)),axis, angle
                    ...
                    if round(found_vectors[2].length(), r) == round(
                        ...
                        if angle0-0.5 < t_angle0 < angle0 + 0.5 and \
                            ...
                            if target_dot < 0 and found_dot < 0 or\
                                ...
                                print vector/targets[0][0]
                                print t_dot0
                                print angle0, t_angle0
                                print angle1, t_angle1
...
def animation_targets(found_vectors, crystal, beam_energy=8, optimise_U=True,
                  refl='orient'):
...
def find_U_matrix(found_vectors, crystal, beam_energy=8, optimise_U=True,
    ...
    # Find the target vectors.
    target_vectors = finding_the_targets(found_vectors, all_vectors)
    print f.find_hkls(crystal, target_vectors,beam_energy, refl=refl)
