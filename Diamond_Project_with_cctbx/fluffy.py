...
def construct_vector(nx_file, item, vector=None):
  ...
  class TransformVisitor(object):
    ...
    def __call__(self, nx_file, depends_on):
      ...
      vector = matrix.col(item.attrs['vector']).normalize()
      if ttype == 'translation':
        value = convert_units(value, units, 'm')
...
class DetectorFactory(object):
  ...
  def __init__(self, obj):
    ...
    fast_pixel_direction_value = convert_units(
      ...
      "m")
    fast_axis = construct_vector(
      nx_file,
      fast_pixel_direction.name).normalize()
    ...
    slow_pixel_direction_value = convert_units(
      ...
      "m")
    slow_axis = construct_vector(
      nx_file,
      slow_pixel_direction.name).normalize()
...
class NXmxModelReader(object):
  ...
  def __init__(self, filename):
    ...
    try:
      ...
      self.errors.append(str(e))
    try:
      self.detector = DetectorFactory(detector).model
    except Exception, e:
      self.detector = None
      self.errors.append(str(e))
    try:
